  ...
  context "Resource" do
    ...
    context "SecurityGroupRedis" do
      ...
      it "to have property SecurityGroupIngress" do
          expect(resource["Properties"]["SecurityGroupIngress"]).to eq([{"FromPort"=>6379, "IpProtocol"=>"tcp", "ToPort"=>6379, "Description"=>{"Fn::Sub"=>"Public access"}, "CidrIp"=>{"Fn::Sub"=>"0.0.0.0/0"}}])
      end
    ...
    context "ParameterGroupRedis" do
      ...
      it "to have property Properties" do
          expect(resource["Properties"]["Properties"]).to eq({"cluster-enabled"=>"yes", "maxmemory-policy"=>"allkeys-lru"})
      end
    ...
    context "ReplicationGroupRedis" do
      ...
      it "to have property Port" do
          expect(resource["Properties"]["Port"]).to eq(1234)
      end
      ...
      it "to have property NumNodeGroups" do
          expect(resource["Properties"]["NumNodeGroups"]).to eq({"Ref"=>"NumNodeGroups"})
      end
      
      it "to have property ReplicasPerNodeGroup" do
          expect(resource["Properties"]["ReplicasPerNodeGroup"]).to eq({"Ref"=>"ReplicasPerNodeGroup"})
      end
      ...
      it "to have property SnapshotWindow" do
          expect(resource["Properties"]["SnapshotWindow"]).to eq("00:30-02:30")
      end
      
      it "to have property PreferredMaintenanceWindow" do
          expect(resource["Properties"]["PreferredMaintenanceWindow"]).to eq("sun:03:25-sun:05:30")
      end
